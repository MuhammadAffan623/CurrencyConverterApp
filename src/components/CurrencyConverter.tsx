...
const CurrencyConverter: React.FC = () => {
  ...
  const handleConvert = async (e: React.FormEvent) => {
    ...
    try {
      const response = await convertCurrency(API_URLS.CONVERT, {
        from,
        to,
        amount: parseFloat(amount),
      });
      if(!response?.data.result || !response.data.rate){
      ...
      setHistory((prev: any) => [conversion, ...prev].slice(0, 10));
    } catch (error) {
  ...
  return (
    <Container className="py-5 ">
      <Card className="glass-card border-0">
        <Card.Body className="px-4 w-full">
